...
namespace kiwi { namespace core { namespace test {
    
    using namespace kiwi;
    using Scheduler = core::Scheduler<std::chrono::high_resolution_clock>;
    ...
    TEST_CASE("Scheduler - Defering tasks", "[Scheduler]")
    {
        ...
        SECTION("Defering task on another thread schedule the task")
        {
            ...
            REQUIRE(!executed_flag);
            REQUIRE(exec_thread.load() != consumer.get_id());
    ...
    {
        using TickScheduler = core::Scheduler<TickClock>;
